...
impl Permutation {
    ...
    pub fn parse_permutation_from_str_arr(str_arr: String) -> Permutation {
        // e.g. [0, 2, 1] -> Permutation::new(vec![1, 3, 2])
        let mut p = Vec::new();
        for s in str_arr.split(",") {
            let s = s.trim();
            let s = s.trim_matches(|c| c == '[' || c == ']');
            p.push(s.parse::<usize>().unwrap());
        }
        return Permutation::new(p);
    }
...
mod permutation_tests {
    ...
    #[test]
    fn test_parse_permutation_from_str_arr() {
        let p = Permutation::parse_permutation_from_str_arr("[0, 1, 2]".to_string());
        assert_eq!(p.p, vec![0, 1, 2]);
    }
    ...
    #[test]
    fn test_pow_compose2() {
        let p = Permutation::new(vec![2, 1, 6, 4, 5, 4, 7, 9, 8]);
        let q = p.pow(10);
        let mut r = Permutation::identity(9);
        for _ in 0..10 {
            r = p.compose(&r);
        }
        assert_eq!(q.p, r.p);

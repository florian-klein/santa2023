...
class Cube {
  ...
  // so we need to paint the colors in this order:
  // D, F, R, B, L, U
  // 5, 0, 1, 2, 3, 4
  //          Positions
  //
...
#pragma region Stats and Info
  ...
  uint64 MoveCount; // Total number of moves made ( rotations of 2 or 3 are
                    // counted as 1 )
...
#pragma endregion
  ...
  void map_move_type_to_action(MoveType move_type);

  void scramble_according_to_move_sequence(std::string move_sequence);
...
#pragma region Solving Centers
  ...
  void OptomizedMovement(uint *mstack, int stkptr, uint r, byte src, byte dst,
                         int sq, int dq);
...
inline void Cube::ExportFrame() {
  ...
  for (byte i = 0; i < 6; i++) {
    std::string filename = "face" + std::to_string(i) + "//F" +
                           std::to_string(i) + "_" +
                           std::string(6 - name.length(), '0') + name + ".png";
...
inline void Cube::Move(const byte face, const int depth, const int q) {
  ...
  switch (face) {
  case 0: // F
    move_sequence.push_back(get_move_string("f", depth, -q, RowSize));
    RotateZ(depth, q);
    return;
  case 1: // R
    move_sequence.push_back(get_move_string("r", depth, -q, RowSize));
    RotateX(R1 - depth, q);
    return;
  case 2: // B
    move_sequence.push_back(get_move_string("f", R1 - depth, q, RowSize));
    RotateZ(R1 - depth, -q);
    return;
  case 3: // L
    move_sequence.push_back(get_move_string("r", R1 - depth, q, RowSize));
    RotateX(depth, -q);
    return;
  case 4: // U
    move_sequence.push_back(get_move_string("d", depth, -q, RowSize));
    RotateY(R1 - depth, -q);
    return;
  case 5: // D
    move_sequence.push_back(get_move_string("d", R1 - depth, q, RowSize));
    RotateY(depth, q);
    return;
  }
}
...
// this is basically a r rotation at index n - index or
inline void Cube::RotateX(const uint index, const int step) {
  if (index == 0)
    faces[3].RotatefaceCW(-step);
  if (index == R1)
    faces[1].RotatefaceCW(step);
...
inline void Cube::RotateY(const uint index, const int step) {
  if (index == 0)
    faces[5].RotatefaceCW(step);
  if (index == R1)
    faces[4].RotatefaceCW(-step);
...
inline void Cube::RotateZ(const uint index, const int step) {
  if (index == 0)
    faces[0].RotatefaceCW(step);
  if (index == R1)
    faces[2].RotatefaceCW(-step);
...
inline double Cube::CurrentProcessDuration() {
  ...
  std::chrono::duration<double, std::ratio<3600>> ProcessTime =
      std::chrono::duration_cast<std::chrono::duration<double>>(
          std::chrono::high_resolution_clock::now() - ProcessStartTime);

...
TinyPngOut::TinyPngOut(uint32_t w, uint32_t h, std::ostream &out)
    : // Set most of the fields
      width(w), height(h), output(out), positionX(0), positionY(0),
      deflateFilled(0), adler(1) {
  ...
  uint64_t uncompRm = (uint64_t)lineSize * (uint64_t)height;
...
void TinyPngOut::write(const uint8_t pixels[], size_t count) {
  ...
  while (count > 0) {
    ...
    if (positionX == 0) { // Beginning of line - write filter method byte
      ...
      if (static_cast<std::make_unsigned<std::streamsize>::type>(
              std::numeric_limits<std::streamsize>::max()) <
          std::numeric_limits<decltype(n)>::max())
        n = std::min(n, static_cast<decltype(n)>(
                            std::numeric_limits<std::streamsize>::max()));
      assert(n > 0);

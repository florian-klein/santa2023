...
const byte Cube::EdgeColorMap[24] = {5, 2, 2, 3, 2, 4, 2, 1, 5, 1, 4, 1,
                                     4, 3, 5, 3, 0, 5, 0, 1, 0, 4, 0, 3};

// Parameters for center commutators
const byte Cube::cmap[30][6] = {
    {0, 1, 4, 5, 3, 3}, {0, 2, 3, 1, 0, 2}, {0, 3, 5, 4, 1, 1},
    {0, 4, 3, 1, 0, 0}, {0, 5, 1, 3, 2, 2}, {1, 0, 5, 4, 1, 1},
    {1, 2, 4, 5, 3, 3}, {1, 3, 5, 4, 1, 1}, {1, 4, 0, 2, 0, 1},
    {1, 5, 2, 0, 2, 1}, {2, 0, 1, 3, 0, 2}, {2, 1, 5, 4, 1, 1},
    {2, 3, 4, 5, 3, 3}, {2, 4, 1, 3, 0, 2}, {2, 5, 3, 1, 2, 0},
    {3, 0, 4, 5, 3, 3}, {3, 1, 4, 5, 3, 3}, {3, 2, 5, 4, 1, 1},
    {3, 4, 2, 0, 0, 3}, {3, 5, 0, 2, 2, 3}, {4, 0, 1, 3, 2, 2},
    {4, 1, 2, 0, 3, 2}, {4, 2, 3, 1, 0, 2}, {4, 3, 0, 2, 1, 2},
...
void Cube::map_move_type_to_action(MoveType move_type) {
  char move_name = move_type.move_name;
  int index = move_type.index;
  int step = move_type.step;
  // std::cout << "(map_move_tyoe_to_action) move_name: " << move_name
  //           << std::endl;
  if (move_name == 'r') {
    RotateX(index, -step);
  } else if (move_name == 'd') {
    ...
    RotateZ(RowSize - index - 1, -step);
  } else {
    std::cout << "Error: move_name " << move_name << " not recognized."
              << std::endl;
    exit(1);
  }
}

void Cube::scramble_according_to_move_sequence(std::string move_sequence) {
  std::vector<MoveType> moves =
      parse_move_string_to_moves(move_sequence, RowSize);
  for (int i = 0; i < moves.size(); i++) {
    map_move_type_to_action(moves[i]);
  }
}
...
void Cube::Scramble(int seed) {
  ...
  for (uint r = 0; r < 3; r++) {
    ...
    for (uint i = 0; i < 3 * RowSize; i++) {
      ...
      if (rnd == 0)
        RotateX(rand() % RowSize, (rand() & 3) + 1);
      if (rnd == 1)
        RotateY(rand() % RowSize, (rand() & 3) + 1);
      if (rnd == 2)
        RotateZ(rand() % RowSize, (rand() & 3) + 1);
    }
...
std::string Cube::Solve() {
  ...
  if (RowSize == 1) {
    ...
    for (byte i = 0; i < 6; ++i)
      faces[i].SetRC(0, 0, i);
    return join_string(move_sequence, "");
  }
...
void Cube::AlignTrueCenters() {
  if (IsEven)
    return; // skip this step if its an even size cube

  byte q;

  // Find Front Center piece
  for (byte i = 0; i < 6; i++) {
    if (faces[i].GetRC(Mid, Mid) == F)
      q = i;
  }
...
void Cube::PushCenterPieces(byte src, byte dst, byte color) {
  ...
  byte srcl = cmap[map][2];    // The face thats 'left' of the src face (in the
                               // direction of the destination)
  int sq = -(int)cmap[map][4]; // Quadrant to use on the source face
  int dq =
      -(int)cmap[map][5]; // The destination is rotated relative to the source
  int d = 1;

  if (IsOpposite(src, dst))
    d = 2;

  uint *mstack = new uint[Mid]; // Temporary array to keep track of columns that
                                // are being moved
  ...
  for (int quadrant = QState; quadrant < 4; ++quadrant) {
    ...
    for (uint r = start; r < R1; ++r) {
      ...
      while (true) {
        ...
        // The row is not clear but has no valid moves (rotate the destination
        // face and continue)
        if (stkptr <= 0) {
          Move(dst, 0, 1);
          continue;
...
void Cube::OptomizedMovement(uint *mstack, int stkptr, uint r, byte src,
                             byte dst, int sq, int dq) {
  byte a, b;
...
void Cube::SolveEdgesOdd() {
  ...
  for (int de = 0; de < 12; ++de) {
    ...
    for (int se = 0; se < 12; ++se) {
      ...
      } while (found); // Repeat the process if more additional pieces are
                       // brought into the edge after moving
...
// Prepare an edge to the source of solved pieces - or put the edge back into
// place set or !set
...
int Cube::FindCorner(int cr) {
  for (int i = 0; i < 8; ++i) {
    if (IsCorner(i, corners[cr][0], corners[cr][1], corners[cr][2]))
      return i;
  }
...
uint64 Cube::PieceCount() {
  ...
  result -=
      ((uint64)RowSize - (uint64)2) * 12; // Edge pieces were counted 2 times
  return result;
}
...
void Cube::Reset() {
  ...
  ProcessStartTime =
      std::chrono::high_resolution_clock::now(); // Reset process timer
}

...
void CubeViewer::ExportFaceDiagram(Face &face, std::string FileName,
                                   int ImageWidth, bool IncludeGridlines) {
  if (ImageWidth <= 0)
    return;

  int pixelcount =
      ImageWidth * ImageWidth;    // number of pixels in the output image
  ...
  if ((uint)ImageWidth <= face.RowSize * 2)
  ...
  for (int y = ImageWidth - 1; y >= 0; --y) {
    for (int x = 0; x < ImageWidth; ++x) {
      ...
      if (IncludeGridlines) {
        ...
        if (((fpx <= linewidth) || (fpx >= (1 - linewidth)) ||
             (fpy <= linewidth) || (fpy >= (1 - linewidth)))) {
          pixels[iptr] = 0x00;
          pixels[iptr + 1] = 0x00;
          pixels[iptr + 2] = 0x00;
        }
  ...
  TinyPngOut pngout(static_cast<uint32_t>(ImageWidth),
                    static_cast<uint32_t>(ImageWidth), out);

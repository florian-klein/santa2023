...
## Permutation groups given by generators. Data structures and basic algorithms.
...
proc parseGensFromArrayFormat*(N: static[int], data: string): GroupGens[N] =
  result = newSeq[Generator[N]]()
  for line in data.splitLines:
    let m = line.match(re"^(\w+):\s+\[(.+)\]")
    if m.isSome:
      let name = m.get.captures[0]
      let permStr = strutils.split(m.get.captures[1], ",")
      var perm: Perm[N]
      for i in 0 ..< N:
        perm[i] = strutils.parseInt(strutils.strip(permStr[i])).Point
      result.add((name, perm, -1))

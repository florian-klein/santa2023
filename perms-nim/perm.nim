...
# ---- string prining elements -----
proc `$`*[N: static[int]](p: Perm[N]): string =
  result = "["
  for i in 0 ..< N:
    result.add($p[i])
    if i < N - 1:
...
proc parsePermFromArrayFormat*(N: static[int], data: string): Perm[N] =
  var parts = N.identity
  var permutationElements = data.findAll(re"\d+")
  for i in 0 ..< N:
    var cur_element = permutationElements[i]
    var part: int
    try:
